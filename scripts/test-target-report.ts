...
import { buildTargetReport, TargetReportResult } from "../lib/utils/monthly-target-report";
...
async function main() {
  ...
  const monthKey = "2026-08"; // override for august
  
  console.log("Fetching targets and assignments for", monthKey);
  ...
  console.log("Fetching technical payments...");
  ...
  // Status logic
  // "status if the month is gone then it status will be met or not met"
  // Is the month over? Yes if todayKey > monthToIso.
  // Wait, todayKey is exactly monthToIso for the last day, or might be inside.
  const isMonthOver = todayKey >= monthToIso;
  ...
  const toEmails = "Abhirup.Kumar@vizvainc.com";
  const subject = `[TEST] Target Report - ${monthKey}`;
  
  await sendNotificationEmail({ to: toEmails, subject, html });
  console.log("Sent successfully to", toEmails);
}
