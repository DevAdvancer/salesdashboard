...
import { getAuditLogFilterOptionsAction } from '@/app/actions/audit-logs';
...
export function AuditLogsClient({ department }: { department?: "sales" | "resume" } = {}) {
  ...
  const [dynamicTargetTypes, setDynamicTargetTypes] = useState<string[]>([]);
  ...
  const fetchNames = async () => {
    try {
      const [references, options] = await Promise.all([
        loadAuditLogReferences(),
        getAuditLogFilterOptionsAction(),
      ]);
      setUsers(references.users);
      setIdToNameMap(references.idToNameMap);
      setDynamicActions(options.actions);
      setDynamicTargetTypes(options.targetTypes);
    } catch (err) {
      console.error('Error fetching data for audit logs:', err);
    }
  };
    ...
    try {
      ...
      let finalActorId: string | string[] | undefined = filterActorId || undefined;

      if (department) {
        const deptUsers = users.filter((u) => (u as any).department === department).map((u) => u.$id);
        if (filterActorId) {
          if (!deptUsers.includes(filterActorId)) {
            finalActorId = "none";
          }
        } else {
          finalActorId = deptUsers.length > 0 ? deptUsers.slice(0, 100) : "none"; // Appwrite Query.equal array limit is 100
        }
      }

      if (finalActorId) auditFilters.actorId = finalActorId;
  ...
  useEffect(() => {
    if (canReadAuditLogs) {
      // If we are filtering by department, we MUST wait for the users array to be populated
      // before fetching logs, otherwise we'll filter with an empty array.
      if (department && users.length === 0) return;
  ...
  }, [canReadAuditLogs, user, filterAction, filterTargetType, filterActorId, filterDateFrom, filterDateTo, users, department]);
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  {users
                    .filter((u) => !department || (u as any).department === department)
                    .map((u) => (
