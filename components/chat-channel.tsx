...
import type { ChatChannelType, ChatMessage, Department } from "@/lib/types";
...
/**
 * Shared chat UI used by both /chat/[channel] (Sales) and
 * /resume-chat/[channel] (Resume). The department is fixed by which route
 * mounted it; the channel comes from the [channel] URL segment.
 *
 * Post authorization: anyone in the active team can post on the "general"
 * channel. The "announcement" channel is admin-only — the server action
 * enforces that, this UI just hides the input.
 */
export function ChatChannelView({ department }: { department: Department }) {
  const { user } = useAuth();
  const params = useParams<{ channel?: string | string[] }>();

  const channel = useMemo<ChatChannelType>(
  ...
  const teamLabel = department === "resume" ? "Resume" : "Sales";
  const pageTitle =
    department === "resume" ? "Resume Team Chat" : "Chatting";
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {teamLabel} team · {getChannelLabel(channel)}
          </p>
        </div>
        <Button
          type="button"
                ...
                <textarea
                  ...
                  placeholder={
                    channel === "announcement"
                      ? `Write a ${teamLabel.toLowerCase()} announcement...`
                      : "Write a message..."
                  }
                  disabled={sending}
                />
