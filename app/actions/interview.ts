...
/**
 * Count interview-support emails SENT within an inclusive ISO date range,
 * scoped to a set of visible lead IDs. Unlike getInterviewAttempts (which
 * merges lifetime totals per lead), this counts each attempt document whose
 ...
 */
export async function countInterviewEmailsSentInRange(
    userId: string,
    role: UserRole,
    branchIds: string[],
    dateFromIso: string,
    ...
    try {
        ...
        // 1. Fetch ALL interview attempts within the date range
        const attempts = await listAllDocuments<InterviewAttemptDocument>({
            databases,
            databaseId: DATABASE_ID,
            collectionId: INTERVIEW_ATTEMPTS_COLLECTION_ID,
            queries: [
        ...
        console.error('Error counting interview emails sent in range:', error);
        return 0;
    }
}
