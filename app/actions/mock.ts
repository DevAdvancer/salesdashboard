...
/**
 * Count mock-request emails SENT within an inclusive ISO date range, scoped
 * to a set of visible lead IDs. Counts each attempt document whose send
 * timestamp (`lastAttemptAt`) falls inside [dateFromIso, dateToIso] so the
 * dashboard reflects "requests sent in this period" regardless of when the
 * underlying lead was created.
 */
export async function countMockEmailsSentInRange(
    userId: string,
    role: UserRole,
    branchIds: string[],
    dateFromIso: string,
    ...
    try {
        ...
        // 1. Fetch ALL mock attempts within the date range
        const attempts = await listAllDocuments<MockAttemptDocument>({
            databases,
            databaseId: DATABASE_ID,
            collectionId: MOCK_ATTEMPTS_COLLECTION_ID,
            queries: [
        ...
        console.error('Error counting mock emails sent in range:', error);
        return 0;
    }
}
